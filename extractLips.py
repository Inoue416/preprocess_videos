...
def extract(path, save_path):
    ...
    if files == []:  # not found data.
        print('Message: ', path, ' is empty.')
        return

    files.sort()  # sort file and load.
    ...
    #fa = face_alignment.FaceAlignment(face_alignment.LandmarksType._2D, flip_input=False, device='cpu')  # if you use CPU
...
def check_lips_length(frame, lip):  # Checking each data length (frames and lips)
    ...
    if frame_len != lip_len:
        record_err_log(lip+' is not frame length.')
...
def make_folder(path):  # This funstion is makeing folder
